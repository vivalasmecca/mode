...
**Component ingestion — external design system on-ramp.** Both phases complete. What's still missing: the ingestion UI (so buyers don't hand-write JSON), and Figma Variables → `color-scale.json` token bridge scripts.
...
- shadcn ingestion Phase 1 + Phase 2 ✓ (component adapter + token bridge; shadcn CSS vars now reference MODE palette vars via globals.css; Studio token changes flow through to shadcn components; font regression from shadcn init fixed; `.neutral` class added to globals.css for MODE's third palette mode)
