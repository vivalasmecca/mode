...
**What the palette driver controls:** In funnel-driven and archetype-driven, the variant dimension drives *both* the palette weights *and* the IA shape — a different funnel stage or archetype produces a different palette map lookup *and* different component selection. In feature-emphasis, the variant dimension (funnel stage) drives IA shape only; the palette map is a flat `{ component → mode }` lookup that is constant across all variants. The art director sets it once ("social proof sections are always dark"); the agent varies what sections appear and in what order per funnel stage.
...
**Token resolver caching:** `token-resolver.js` caches `mode-tokens.json` at module load time (Node.js module cache). Editing palette maps or accent tokens via the Palette tab writes to the file immediately, but a dev server restart is needed for those changes to take effect in new builds. The UI notes this after saving. **Restart is also required after any manual edit to `mode-tokens.json`** — including switching active preset or editing palette maps outside the UI. The Studio palette map panel (`PUT /api/palette`) writes the file but does not reload the cache; a restart is required before those changes affect new builds.
...
**Brief `goal` field goes directly to the LLM.** The `goal` in `shared_brief` (in `config/site.json` or `config/site-feature.json`) is injected verbatim into the IA planner prompt. It must read as a real marketing objective ("convince SaaS teams that..."), not as documentation or a system description. If it describes the preset architecture instead of a page purpose, the IA planner will generate a minimal structural page rather than a full marketing page. This is a real failure mode — the LLM takes it literally.
...
└── ui/                              ← Next.js 16 dashboard + preview
    ...
    │   ├── modules/                 ← one component per manifest entry (or adapter)
    │   │   └── index.ts             ← MODULE_REGISTRY
    │   ├── external/                ← buyer-side components (no MODE knowledge; wrapped by adapters in modules/)
    │   │   └── ShadcnContentSection.tsx  ← shadcn Card composition (Phase 1 ingestion test)
    │   ├── ui/                      ← shadcn/Base UI generated components (card, button)
...
4. **After any palette map or token edit:** restart the dev server (`Ctrl-C` → `npm run dev`) before running a new build. `token-resolver.js` caches `mode-tokens.json` at startup; saves via the Palette tab or Studio do not reload the cache.
5. **If you get a "Cannot find module 'mode-agent'" error:** run `npm install` from `ui/` to restore the symlink.
6. **If the dashboard throws a JSON parse error on load:** delete `.next/` and restart (`rm -rf .next && npm run dev`).
...
- **Feature-emphasis Studio mode** ✓ — When `palette_driver = component_role`, Studio expanded view shows a Palette map panel instead of the Token panel. Clicking any component's badge cycles its palette mode (light → neutral → dark); changes patch `section.palette` across all in-memory page variants instantly — all pages update without a rebuild. Save writes to `mode-tokens.json` via `PUT /api/palette`. Save button enables when either palette map or component overrides are pending.
- **Full component palette compliance** ✓ — All 11 module components use `getPalette()` and apply palette vars to their outer wrapper. `NavigationHeader` and `FooterMinimal` previously hardcoded `bg-white` and ignored the palette prop; both now respond to mode changes. Nav links, footer text, separators, and borders all use palette vars.
- **Neutral mode visually distinct** ✓ — `theme.json` `neutral.bg` changed from `white` to `slate.2`. Previously neutral and light rendered identically; switching a component to neutral appeared broken even when correctly wired.
...
**Component ingestion — external design system on-ramp.** The minimum viable ingestion path is now built and validated with a real component. The CLI workflow lets a buyer write a declaration JSON and generate an adapter in one command — confirmed working with the shadcn Phase 1 test (see below). What's still missing: the ingestion UI (so buyers don't hand-write JSON), and token bridge scripts (Figma Variables → `color-scale.json`). The token bridge (Phase 2) is the next ingestion milestone.

**shadcn test — Phase 1 complete, Phase 2 pending.** Phase 1 (component adapter layer) is done and validated:
- shadcn v4 with Base UI (not Radix UI) installed in `ui/`; Card + Button from `@base-ui-components/react`
- `ShadcnContentSection` wrapper at `ui/components/external/` composes Card + CardHeader + CardContent from shadcn
- Declaration at `declarations/shadcn-content.json`; adapter generated by `node agent/ingest.js declarations/shadcn-content.json --write`
- **Key finding: className is the only palette bridge mechanism** — shadcn has no `theme` prop. `dark` class scopes shadcn's CSS vars to the element and descendants (`--card`, `--foreground`, etc. flip to dark values). Neutral uses `bg-muted` class. Both expressed as `{ className: "..." }` in the palette bridge.
- `@custom-variant dark (&:is(.dark *))` in globals.css defines the `dark:` Tailwind prefix to target children of `.dark` — so `.dark` on the section element correctly scopes all shadcn CSS vars for both the element itself and its descendants.
- TypeScript type check passes (`npx tsc --noEmit` from `ui/`). Manifest validator confirms all 11 components registered.

Phase 2 (token bridge) is the remaining milestone: replace `color-scale.json` with shadcn's oklch CSS variable values and update `theme.json` to reference shadcn's semantic tokens (`--card`, `--foreground`, etc.) so MODE's palette modes flow through shadcn's rendering without per-component className overrides. Do this on a branch — it temporarily breaks MODE's own components.
...
- Feature-emphasis preset correctly implemented ✓ (component-role driver; flat `{ component: mode }` palette map; `isFlat` flag in Palette tab UI; `config/site-feature.json` for site builds; funnel_stage as variant dimension — IA shape varies per stage, palette constant across all stages)
...
- shadcn ingestion Phase 1 ✓ (real external component validated end-to-end: `declarations/shadcn-content.json` → adapter → `ContentSection` replaced; className palette bridge confirmed; shadcn v4 + Base UI + Tailwind v4 compatible)
