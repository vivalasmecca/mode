...
**shadcn test — two phases, not one.** The planned shadcn test validates the component adapter layer (slot mapping, variant mapping, palette bridge at the prop level). It does NOT validate the expression layer handoff, which is what a real buyer actually brings. A full buyer scenario is three things: (1) component implementations, (2) their color vocabulary replacing `color-scale.json`, and (3) their semantic token assignments replacing `theme.json`. Phase 1 of the test = shadcn component → adapter → renders in Studio. Phase 2 = replace `color-scale.json` with shadcn's CSS variable values + update `theme.json` to reference shadcn's semantic tokens, then verify MODE's palette modes flow through shadcn's rendering correctly. Both phases must pass for a buyer to genuinely adopt MODE without modifying MODE's core. Phase 2 is the token bridge test — currently undocumented and untested.
